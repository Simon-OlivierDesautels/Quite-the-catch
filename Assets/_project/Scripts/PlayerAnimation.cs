...
public class PlayerAnimation : MonoBehaviour
{
    ...
    private void Walk()
    {
        ...
        if (_horizontalInput != 0) ChangeAnimationState(Animation.Walk.ToString());
        else ChangeAnimationState(Animation.Idle.ToString());
    }
    private void Run()
    {
        if (_horizontalInput < -_runningSpeed || _horizontalInput > _runningSpeed) ChangeAnimationState(Animation.Run.ToString());
    }
